...
const KIND_LABEL = {
  meeting: 'reunião', entity: 'empresa / produto', person: 'pessoa', concept: 'conceito', page: 'página', document: 'documento-fonte',
  sync: 'reunião recorrente', qa: 'perguntas e respostas', 'all-hands': 'reunião geral', planning: 'planejamento', interview: 'entrevista', call: 'chamada', community: 'encontro da comunidade', recording: 'gravação', series: 'série de reuniões',
  video: 'vídeo', tool: 'ferramenta', technique: 'técnica', comparison: 'comparação',
  feedback: 'memória de feedback', project: 'memória de projeto', reference: 'memória de referência', user: 'sobre você', note: 'nota',
  recollection: 'resumo de sessão',
  app: 'app / ferramenta', business: 'negócio e operação', bucket: 'saída de skill', skill: 'skill', agent: 'agente', other: 'projeto', entidade: 'entidade', conceito: 'conceito', fonte: 'fonte',
};
const FLAG_TEXT = {
  stale: (n, s) => `Desatualizada: última alteração ${ago(n.updated)} (limite de ${s.staleDays} dias)`,
  quiet: (n) => `Parada: sem alterações há ${n.ageDays} dias. Arquive ou retome.`,
  orphan: () => 'Órfã: nada aponta para cá e ela não aponta para nada',
  stub: (n) => `Esboço: só ${n.words} palavras`,
  'broken-links': (n) => (n.broken.length === 1 ? '1 link aponta para uma página que não existe' : `${n.broken.length} links apontam para páginas que não existem`),
  'missing-folder': () => 'Listada no índice, mas a pasta não existe',
  'not-ingested': () => 'Só transcrição: ainda sem página na wiki. Ingira ou deixe como registro.',
  archived: () => 'Marcada como arquivada no cabeçalho',
};
...
function ago(ts) {
  ...
  return `há ${(d / 365).toFixed(1)} anos`;
}
function fmtDate(ts) {
  if (!ts) return '';
...
async function main() {
  ...
  data = await res.json();
  if (!res.ok) throw new Error(data.error || 'Não foi possível carregar o cérebro.');
  ...
  $('visible-status').textContent = data.warnings?.length ? data.warnings.length + (data.warnings.length === 1 ? ' aviso de fonte' : ' avisos de fonte') + ' no inventário' : 'Conhecimento local · conectado';
  ...
  renderLegend();
  renderInventory();
  $('brand-meta').textContent = `${num(data.nodes.length)} notas · ${num(data.links.length)} conexões`;
  $('splash-stats').textContent = `${num(data.nodes.length)} notas · ${num(data.links.length)} conexões · ${data.sources.length} fontes`;
...
function onHover(n) {
  ...
  if (n && (!selected || n.id !== selected.id)) {
    hl.innerHTML = `<b>${esc(n.title)}</b><span>${esc(sourceById[n.source].label)} · ${esc(KIND_LABEL[n.kind] || n.kind)}${n.updated ? ' · ' + esc(ago(n.updated)) : ''}${n.flags?.length ? ' · precisa de atenção' : ''}</span>`;
    hl.hidden = false;
...
function renderDrawer(n) {
  ...
  const meta = [];
  if (n.updated) meta.push(`<span>atualizada <b>${fmtDate(n.updated)}</b> (${ago(n.updated)})</span>`);
  if (n.created && n.created !== n.updated) meta.push(`<span>criada <b>${fmtDate(n.created)}</b></span>`);
  if (n.words) meta.push(`<span><b>${num(n.words)}</b> palavras</span>`);
  meta.push(`<span><b>${n.degree}</b> conexões</span>`);
  if (n.views != null) meta.push(`<span><b>${num(n.views)}</b> visualizações</span>`);
  if (n.likes != null) meta.push(`<span><b>${num(n.likes)}</b> curtidas</span>`);
  if (n.channel) meta.push(`<span>${esc(n.channel)}</span>`);
  if (n.source === 'meeting' && n.kind !== 'series') {
    if (n.meetingDate) meta.unshift(`<span>realizada <b>${fmtDate(n.meetingDate)}</b> (${ago(n.meetingDate)})</span>`);
    if (n.seriesGroup || n.series) meta.push(`<span>série <b>${esc(n.seriesGroup || n.series)}</b></span>`);
    if (n.duration) meta.push(`<span><b>${n.duration}</b> min</span>`);
    if (n.transcriptWords) meta.push(`<span>transcrição <b>${num(n.transcriptWords)}</b> palavras</span>`);
    meta.push(`<span>${n.ingested ? 'com página na wiki' : 'só transcrição'}</span>`);
    if (n.participants?.length) meta.push(`<span>com <b>${esc(n.participants.slice(0, 8).join(', ') || 'não listados')}</b></span>`);
  }
  if (n.kind === 'series') meta.push(`<span><b>${(n.seriesMembers || []).length}</b> reuniões</span><span><b>${n.ingestedCount || 0}</b> com página</span>`);
  if (n.published && n.kind === 'video') meta.push(`<span>publicado <b>${fmtDate(n.published)}</b></span>`);
  if (n.status) meta.push(`<span>status <b>${esc(n.status)}</b></span>`);
  if (n.confidence) meta.push(`<span>confiança <b>${esc(n.confidence)}</b></span>`);
  if (n.model) meta.push(`<span>modelo <b>${esc(n.model)}</b></span>`);
  ...
  if (!flags.length) flags.push('<span class="flag ok">Saudável: ligada, atual e completa</span>');
  ...
  tr.textContent = 'Ler transcrição';
  ...
  for (const s of data.sources) {
    ...
    if (list.length > 40) rows.push(`<div class="conn muted"><span>… e mais ${list.length - 40}</span></div>`);
  ...
  $('d-connections').innerHTML = parts.join('') || '<div class="muted small-text">Sem conexões. Ligue esta nota a partir de outra, ou ela ficará invisível para a recuperação.</div>';
...
async function readNote(n, part = 'note') {
  ...
  const label = part === 'transcript' ? 'Ler transcrição' : 'Ler nota';
  if (!wrap.hidden && shownPart === part) { wrap.hidden = true; shownPart = null; btn.textContent = label; return; }
  $('d-read').textContent = 'Ler nota';
  $('d-transcript').textContent = 'Ler transcrição';
  ...
  md = md.replace(/\[\[([^\]|#]+)(?:#[^\]|]*)?(?:\|([^\]]*))?\]\]/g, (m, target, alias) => {
    ...
    return t ? `<a class="wikilink" data-id="${esc(t.id)}">${text}</a>` : `<a class="wikilink dead" title="nenhuma página chamada ${esc(target)}">${text}</a>`;
  });
  let html = '';
  ...
  for (const anchor of $('d-content').querySelectorAll('a[href]')) {
    ...
    else { anchor.classList.add('dead'); anchor.title = 'Destino fora do grafo selecionado ou não resolvido'; }
  }
  $('d-path').textContent = body.path || n.path;
  wrap.hidden = false;
  shownPart = part;
  btn.textContent = part === 'transcript' ? 'Ocultar transcrição' : 'Ocultar nota';
...
function applyVisibility() {
  ...
  restyleAll();
  $('visible-status').textContent = `${num(data.nodes.filter(isVisible).length)} notas em exibição`;
}
...
function renderInventory() {
  ...
  $('inv-lede').textContent = `${num(inv.totals.nodes)} notas e ${num(inv.totals.links)} conexões em ${srcs.length} fontes, cerca de ${num(Math.round(inv.totals.words / 1000))} mil palavras. Clique em uma fonte para vê-la sozinha. Todas as listas abaixo são clicáveis.`;
  ...
  $('inv-sources').innerHTML = srcs.map((s) => {
    ...
    return `<div class="inv-card" data-source="${s.id}" style="--c:${s.color}">
      ...
      <div class="inv-card-foot">
        <span>mais nova <b>${s.newest ? ago(s.newest) : '?'}</b></span>
        <span>mais antiga <b>${s.oldest ? ago(s.oldest) : '?'}</b></span>
        ${s.words ? `<span><b>${num(Math.round(s.words / 1000))} mil</b> palavras</span>` : ''}
        <span class="${s.flagged ? 'warn' : ''}"><b>${s.flagged}</b> precisam de atenção</span>
  ...
  $('inv-sources').addEventListener('click', (e) => {
    ...
    syncLegend();
    applyVisibility();
    toast(activeSources.size === 1 ? `Mostrando só ${sourceById[id].label}` : 'Mostrando tudo');
  });
  ...
  if (yt.videos) {
    ...
    $('inv-youtube').innerHTML = `
      <div class="section-head"><span>Cobertura do YouTube</span><span class="count">${pct}% do canal está no cérebro</span></div>
      <div class="yt-grid">
        <div class="yt-tile"><div class="l">Vídeos do canal</div><div class="v">${num(yt.videos)}</div><div class="m">${num(yt.subscribers)} inscritos</div></div>
        <div class="yt-tile"><div class="l">Transcritos</div><div class="v">${num(yt.transcribed)}</div><div class="m">páginas na wiki</div></div>
        <div class="yt-tile"><div class="l">Não ingeridos</div><div class="v">${num(yt.videos - yt.transcribed)}</div><div class="m">ingerir com /wiki</div></div>
      </div>
  ...
  if (mt && mt.total) {
    ...
    const rows = mt.series.map((s) => `<div class="hrow" data-id="${esc(s.hub)}"><i style="background:var(--meeting)"></i><span class="t">${esc(s.series)}</span><span class="a">${s.total} · ${s.ingested} na wiki · ${s.first ? fmtDate(s.first).slice(0, 7) : ''} a ${s.last ? fmtDate(s.last).slice(0, 7) : ''}</span></div>`).join('');
    $('inv-meetings').innerHTML = `
      <div class="section-head"><span>Reuniões</span><span class="count">${num(mt.total)} reuniões · ${num(Math.round(mt.minutes / 60))} horas</span></div>
      <div class="yt-grid">
        <div class="yt-tile"><div class="l">Com página</div><div class="v">${num(mt.ingested)}</div><div class="m">páginas da wiki com transcrição</div></div>
        <div class="yt-tile"><div class="l">Só transcrição</div><div class="v">${num(mt.transcriptOnly)}</div><div class="m">ainda sem página na wiki</div></div>
        <div class="yt-tile"><div class="l">Última página</div><div class="v">${gapDays == null ? '?' : gapDays + 'd'}</div><div class="m">${mt.lastIngested ? 'atrás, ' + fmtDate(mt.lastIngested) : 'nunca'}</div></div>
      </div>
      ...
      <p class="muted small-text" style="margin:10px 0 4px">${fmtDate(mt.first)} a ${fmtDate(mt.last)}. Cada série abaixo é um nó central no grafo; clique para abrir.</p>
      <div class="rows" style="max-height:none">${rows}</div>`;
  } else $('inv-meetings').hidden = true;

  const flagDefs = [
    ['not-ingested', 'Reuniões sem página', 'A transcrição está nas fontes, mas não existe página na wiki. Ingira as que importam; o resto pode ficar como registro.'],
    ['stale', 'Páginas desatualizadas', 'Páginas vivas sem atualização além do limite. Releia e atualize, ou arquive.'],
    ['quiet', 'Projetos parados', 'Sem alterações há 60 dias. Mova para arquivo/ e tire do índice, ou retome.'],
    ['broken-links', 'Links quebrados', 'Nenhum destino inequívoco nas fontes selecionadas. Confira o link ou adicione a fonte que falta.'],
    ['orphan', 'Órfãs', 'Nada aponta para estas. A recuperação nunca vai achá-las seguindo links.'],
    ['stub', 'Esboços', 'Menos de 60 palavras. Amplie se for útil, ou arquive.'],
    ['missing-folder', 'Pastas ausentes', 'Listadas no índice de projetos, mas a pasta sumiu.'],
    ['archived', 'Marcadas como arquivadas', 'O cabeçalho diz arquivada; considere mover o arquivo.'],
  ];
  let total = 0;
  $('inv-health').innerHTML = flagDefs.filter(([key]) => ['stale','orphan','stub','broken-links'].includes(key)).map(([key, title, blurb]) => {
    ...
    const rows = ids.map((id) => nodesById.get(id)).filter(Boolean)
      ...
      .join('');
    return `<details class="health"><summary><span>${esc(title)}<small>${esc(blurb)}</small></span><span class="n ${ids.length ? '' : 'ok'}">${ids.length}</span></summary><div class="rows">${rows || '<div class="muted small-text" style="padding:6px 8px">Nada aqui.</div>'}</div></details>`;
  }).join('');
  $('inv-attention-count').textContent = `${num(total)} itens`;
  ...
  $('inv-foot').innerHTML = `<span>grafo gerado em ${new Date(inv.generatedAt).toLocaleString('pt-BR')}</span><button class="link-btn" id="btn-rebuild">Reconstruir do disco</button>`;
}
...
function renderSearch(list, q) {
  ...
  box.hidden = false;
  if (!list.length) { box.innerHTML = '<div class="result-empty">Nenhuma nota corresponde. Tente uma pessoa, uma ferramenta ou um título.</div>'; return; }
...
function stopDemo() {
  ...
  $('btn-demo').textContent = 'Ver crescimento';
...
function toggleDemo() {
  ...
  if (!eligible.length) { toast('Escolha uma fonte com notas para ver o crescimento.'); return; }
...
function updateDemo() {
  ...
  if (growthTime-growthLastPaint > .12 || growthLastPaint < 0 || progress === 1) {
    ...
    $('demo-source').textContent = `O crescimento de ${data.brain.name}`;
    $('demo-title').textContent = growthTime < 1.2 ? 'Começa com uma ideia.' : growthTime < 4 ? 'Depois, uma conexão.' : growthTime < 12 ? 'Ideias viram ramos.' : growthTime < 22 ? 'O conhecimento se acumula.' : progress < 1 ? 'Um segundo cérebro toma forma.' : 'Seu cérebro inteiro. Conectado.';
    $('demo-detail').textContent = `${sourceCount} ${sourceCount === 1 ? 'fonte' : 'fontes'} · ${num(live.length)} de ${num(growth.records.length)} notas salvas`;
    ...
    $('visible-status').textContent = `${num(live.length)} notas em exibição`;
  }
  if (progress === 1) {
...
function setMotion(value) {
  ...
  $('btn-rotate').title = value ? 'Pausar movimento' : 'Retomar movimento';
  graph.linkDirectionalParticles(particlesFor);
...
function toggleCinema() {
  ...
  $('btn-cinema').textContent = cinematic ? 'Sair do cinema' : 'Cinema';
...
function wireUi() {
  ...
  $('btn-rotate').addEventListener('click', () => {
    ...
    toast(autoRotate ? 'Movimento ligado' : 'Movimento pausado');
  });
  $('toggle-attention').addEventListener('change', (e) => { attentionOnly = e.target.checked; applyVisibility(); if (attentionOnly) toast('Mostrando só notas que precisam de atenção'); });
  ...
  $('d-reveal').addEventListener('click', async () => {
    ...
    toast('Aberto no gerenciador de arquivos');
  });
  ...
  $('inventory').addEventListener('click', async (e) => {
    ...
    if (e.target.id === 'btn-rebuild') {
      e.target.textContent = 'Reconstruindo…';
...
main().catch((err) => {
  console.error(err);
  $('splash-stats').textContent = `Falha ao carregar: ${err.message}`;
});
