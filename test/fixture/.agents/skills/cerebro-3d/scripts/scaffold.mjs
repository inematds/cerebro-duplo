...
if(!input) throw new Error('Uso: node scaffold.mjs --root <pasta do cérebro> --config <arquivo JSON> [--out <pasta nova do app>]');
const out=path.resolve(root,value('--out')||'apps/cerebro-3d');
if(!out.startsWith(root+path.sep)) throw new Error('A saída precisa ser uma pasta nova dentro do cérebro escolhido.');
try{await fs.access(out);throw new Error('A pasta de saída já existe. Reaproveite a configuração dela ou mova para arquivo/ antes de instalar outra cópia.');}catch(err){if(err.code!=='ENOENT')throw err;}
...
console.log(JSON.stringify({app:out,name:config.name,port:config.port,next:'Rode node build.mjs e depois node serve.mjs dentro da pasta do app.'},null,2));
