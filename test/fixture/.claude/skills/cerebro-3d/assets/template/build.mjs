...
const blocked = new Set(['.git','node_modules','dist','build','archives','archive','arquivo','tmp','audits','auditorias','data','assets','scripts','tests','examples','apps']);
...
export async function buildGraph(configFile = path.join(HERE,'brain.config.json')) {
  ...
  async function add(file, source, input = {}) {
    ...
    // README só é página de navegação quando está na raiz da pasta selecionada (ex.: contexto/README.md).
    const nav = /^(_index.*|_hot|_log.*|index|log|overview)$/i.test(base) || (/^readme$/i.test(base) && !!input.base && path.dirname(real) === input.base);
    const id = `${source.id}:${hash(relative+':'+(input.title || input.startLine || ''))}`;
    const title = String(input.title || fm.title || fm.titulo || fm.name || heading(body) || path.basename(real,path.extname(real)).replace(/[-_]/g,' ')).replace(/\s+/g,' ').trim();
    const date = Date.parse(fm.updated || fm.atualizado || fm['last-updated'] || '');
    const node = {id,source:source.id,kind:nav ? 'index' : input.kind || fm.type || fm.tipo || 'note',title,slug:key(path.basename(real)),summary:fm.description || fm.descricao || summary(body),path:within(real,root) ? relative : slash(real),relativePath:relative,inRepo:within(real,root),updated:Number.isFinite(date)?date:stat.mtimeMs,created:Date.parse(fm.created || fm.criado || '') || null,words:(body.match(/\S+/g)||[]).length,tags:(fm.tags||'').replace(/[\[\]'"]/g,'').split(',').map(s=>s.trim()).filter(Boolean),degree:0,flags:[],broken:[],startLine:input.startLine,endLine:input.endLine};
  ...
  async function walk(dir,source,base=dir) {
    ...
    for (const e of (await fs.readdir(dir,{withFileTypes:true})).sort((a,b)=>a.name.localeCompare(b.name))) {
      ...
      else if (/\.(md|txt)$/i.test(e.name)) await add(file,source,{base:await fs.realpath(base)});
    }
  }
...
if (process.argv[1] && path.resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  ...
  console.log(`${graph.brain.name}: ${graph.nodes.length} notas, ${graph.links.length} conexões, ${graph.sources.length} categorias.`);
